...
# def get_kaggle_recommendations(retriever, pdf_text):
...
#                 QUERY_TEMPLATE = prompt_constructor(selected_action, pdf_text)
...
def get_kaggle_recommendations(retriever,docx_text):
        # Create QA_CHAIN_PROMPT from the template
    ...
    with st.spinner("Wait.Fetching kaggle datasets recommendations..."):
        ...
        print("-"*200)
        print(QUERY_TEMPLATE)
        string_response = generating_response(QUERY_TEMPLATE, retriever, docx_text)
...
if uploaded_file is not None:
    ...
    st.markdown(docx_text)
    st.session_state.is_docx_file_uploaded = True
    
if st.session_state.is_docx_file_uploaded:
    ...
    if not st.session_state.is_recommendation_generated:
        get_kaggle_recommendations(retriever, docx_text)
